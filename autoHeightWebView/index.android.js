...
export default class AutoHeightWebView extends PureComponent {
  ...
  onMessage(e) {
    const messageObj = JSON.parse(IsBelowKitKat ? e.nativeEvent.message : e.nativeEvent.data);
    if (messageObj.tag === 'height') {
      const height = messageObj.data;
      if (height) {
        if (this.props.enableAnimation) {
  ...
  : `
    ; (function () {
        document.addEventListener('message', function (e) {
            window.postMessage(String('{ "tag": "height", "data": ' + document.body.offsetHeight + ' }'));
        });
    } ());
    `;
